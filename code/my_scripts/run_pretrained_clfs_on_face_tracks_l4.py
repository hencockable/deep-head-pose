...
face_detection_df = pd.read_csv("/home/hendrik/PycharmProjects/Master/Hopenet/source/face_detection/{}_l4.csv".format(file))
...
for vp in vps:
    ...
    for _, row in face_track_df.iterrows():
        if row.frame_num in face_detection_df.frame_num.values and row.face_id in face_detection_df.face_id.values:
            l4 = face_detection_df.loc[(face_detection_df.frame_num == row.frame_num) & (face_detection_df.face_id == row.face_id)].l4.values[0][1:-1].replace("\n", "").replace("  ", " ").split(" ")
            try:
    ...
    pred_17 = list(clf_17.predict(X))
    pred_all = list(clf_all.predict(X))
